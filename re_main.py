...
class Trainer:
    ...
    def decode(self, start, end, index):
        res = []

        for st, en, ind in zip(start, end, index):
            p = ind.index(1)
            flag = True
            for_flag = False
            try:
                for i in range(len(st)):
                    for j in range(len(en)):
                        if i >= p and j >= p and st[i] == en[j] and st[i] == 1:
                            res.append([i, j])
                            flag = False
                            for_flag = True
                            break
    ...
    def test(self):
        ...
        for step, batch_data in enumerate(tqdm(self.test_loader)):
            ...
            index = np.where(start_labels != -100, 1, 0).tolist()

            pred = self.decode(start_logits, end_logits, index)
            true = self.decode(start_labels, end_labels, index)
